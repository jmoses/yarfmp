module YARFMP
  ...
  def self.included( base )
    base.class_eval do
      ...
      helper_method :has_flash_messages?
    end
  end
    ...
    def render_flash_messages
      ...
      content_tag(:div, :id => "flash_messages_wrap") do 
        content_tag(:div, :id => "flash_messages") do
          YARFMP::InstanceMethods::FLASH_MESSAGE_LEVELS.collect do |level|
            render_flash_message_single(flash[YARFMP::InstanceMethods::FLASH_MESSAGE_KEY][level], :class => "message_#{level}")
          end.join("\n")
        end
      end
    end
...
ActionController::Base.send(:include, YARFMP )
ActionView::Base.send(:include, YARFMP::InstanceMethods ) ## This only works once!?????